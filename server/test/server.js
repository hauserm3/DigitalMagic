...
var getDevices = function (req, res, next) {
    console.log('GD');
    var http_req = http.request(optionsDevices, function (response) {
        response.setEncoding('utf8');
        var rawData = '';
        response.on('data', function (chunk) {
            rawData += chunk;
...
var getDevicesConnection = function (req, res, next) {
    console.log('GDС');
    var http_req = http.request(optionsDevices, function (response) {
        response.setEncoding('utf8');
        var rawData = '';
...
var getDevicesDisconnection = function (req, res, next) {
    console.log('GDD');
    var http_req = http.request(optionsDevices, function (response) {
        response.setEncoding('utf8');
        var rawData = '';
...
app.get('/getAllDevices2', function (req, res) {
    ...
    var deviceIds = function (allDevices) {
        var dIds = '';
    ...
    var http_req = http.request(options, function (response) {
        ...
        response.on('end', function () {
            ...
            parseString(rawData, function (err, result) {
                ...
                var devicePlayingContent = result.response.responseClass[0];
                var data = querystring.stringify({
                    service: 'CommonContentService.getContentInfo',
                    token: token,
                    ...
                    response.on('end', function () {
                        ...
                        parseString(rawData, function (err, result) {
                            ...
                            if (result.response.responseClass[0].media_type[0] == 'MOVIE') {
                                ...
                                var data_1 = querystring.stringify({
                                ...
                                var options_1 = {
                                    ...
                                    headers: {
                                        'Connection': 'keep-alive',
                                        'Content-Type': 'application/x-www-form-urlencoded',
                                        'Content-Length': Buffer.byteLength(data_1),
                                        'User-Agent': 'Mozilla/5.0'
                                    }
                                };
                                var http_req_1 = http.request(options_1, function (response) {
                                    response.setEncoding('utf8');
                                    var rawData = '';
                                    response.on('data', function (chunk) {
                                        rawData += chunk;
